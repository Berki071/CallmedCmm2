    ...
    fun getData() {
        ...
        for(i in allFiles){
            if(i.name.contains(searchStr)){
                if(i.extension == "jpeg" || i.extension == "jpg" || i.extension == "png"){
                    listImage.add(i)
                }
            }
        }

        listImageWithDate = processingData(listImage)
